 ...
 function handleComplete(event) {
     ...
     var ball = new createjs.Shape().set({
         alpha: 0.4
     });
     ball.graphics.beginFill("#ffffff").drawRoundRect(170, 206, 10, 33, 10);
     ball.graphics.endFill();

     var tween = createjs.Tween.get(ball).wait(10000)
         .to({
             x: 115,
             y: 0,
             alpha: 0
     ...
     frame2.addChild(join, applies);
     frame3.addChild(button, year, when, offer, rental, ball);
     ...
     createjs.Tween.get(stamp).wait(2300).to({
         alpha: 1
     }, 4000).call(bounce, {});
 ...
 function fade() {
     createjs.Tween.get((currentImage == frame1) ? frame2 : frame1).wait(200).to({
         alpha: 1
     }, 1000);
     ...
     createjs.Tween.get(ball).wait(1).to({
         alpha: 0
     }, 1).call(fadeKill);
